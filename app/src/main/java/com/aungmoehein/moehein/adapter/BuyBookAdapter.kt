...
class BuyBookAdapter(val context: Context, val booklist: List<Buy>): RecyclerView.Adapter<BuyBookAdapter.BuyWriterBookViewHolder>() {
    val db = MoeHein.getInstance(context)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BuyBookAdapter.BuyWriterBookViewHolder {
        return BuyWriterBookViewHolder(LayoutInflater.from(context).inflate(R.layout.writer_other_list,parent,false),this,context)
    }
    ...
    override fun onBindViewHolder(holder: BuyBookAdapter.BuyWriterBookViewHolder, position: Int) {
    ...
    class BuyWriterBookViewHolder(itemView: View, adapter: BuyBookAdapter, val context: Context) : RecyclerView.ViewHolder(itemView),View.OnClickListener{
        ...
        override fun onClick(v: View?) {
            ...
            dialog.details_buy_qty.text = MDetect.getText("အရေအတွက် - "+ myanNum(adapter.booklist[adapterPosition].quantity.toString())+"အုပ်")
